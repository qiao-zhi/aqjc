(function($) {
	...
	$.addFlex = function(t, p) {
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function(i, row) {
								...
								if (p.checkbox) {
									...
									// {//add matychen 不要noborder，加入后ie8显示有问题。
		...
		if (p.colModel) {
			...
			for (i = 0; i < p.colModel.length; i++) {
				...
				//start 杜媛 2013.07.12
				if($('#jspName').val()=="emergencyAComList"||$('#jspName').val()=="emergencyAList"||$('#jspName').val()=="emergencyList"||$('#jspName').val()=="emergencyComList"){
					$(th).attr('id', cm.name);
				}
				//end
		...
		if (p.buttons) {
			...
			for (i = 0; i < p.buttons.length; i++) {
				var btn = p.buttons[i];
				if (!btn.separator&&!btn.separatorExcel) {
				...
				}else if(btn.separatorExcel) {
					$(tDiv2).append("<div class='btnseparatorExcel'></div>");
				}else{
					$(tDiv2).append("<div class='btnseparator'></div>");
				}
			}
			$(g.tDiv).append(tDiv2);
			$(g.tDiv).append("<div class='wjtitle'></div>");
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton" title="首页"><span></span></div><div class="pPrev pButton" title="上一页"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'
					+ p.pagetext
					+ ' <input type="text" size="4" value="1" id="pages" /> '
					+ p.outof
					+ ' ,共<span> 1 </span>页</span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton" title="下一页"><span></span></div><div class="pLast pButton" title="尾页"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton" title="刷新"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv)
						.append('<div class="ptogtitle" title="最小化/最大化 表格"><span></span></div>');
	...
	$.fn.flexReload = function(p) { // function to reload grid
		...
		return this.each(function() {
			if(p){
				this.p.wjsearch=$.extend(this.p.wjsearch,p.wjsearch);
			}else{
				this.p.wjsearch
			}
