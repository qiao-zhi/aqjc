...
function pageonload() {    
    ...
    if (twainsource) {
        ...
        twainsource.options.add(new Option("正在寻找设备，请稍等.", 0));
...
function InitBtnGroupBtm(bNeebBack) {
    ...
    if (btnGroupBtm) {
		...
		objString += "<div class='ct-rt'>预览格式";
...
function InitDWTdivMsg(bNeebBack) {
    ...
    if (DWTemessageContainer) {
		...
		if (bNeebBack) {
		objString += "<p class='backToDemoList'><a class='btnOrg' href =\"online_demo_list.aspx\">Back</a></p>";
		}
...
function initCustomScan() {
    ...
    ObjString += "<li>";
    ObjString += "<label id ='lblShowUI' for = 'ShowUI'><input type='checkbox' id='ShowUI' />显示用户界面&nbsp;</label>";
    ObjString += "<label for = 'ADF'><input type='checkbox' id='ADF' />自动进纸&nbsp;</label>";
    ObjString += "<label for = 'Duplex'><input type='checkbox' id='Duplex'/>双面扫描</label></li>";
    ObjString += "<li>扫描类型:";
    ObjString += "<label for='BW' style='margin-left:5px;'><input type='radio' id='BW' name='PixelType'/>黑白 </label>";
    ObjString += "<label for='Gray'><input type='radio' id='Gray' name='PixelType'/>灰色</label>";
    ObjString += "<label for='RGB'><input type='radio' id='RGB' name='PixelType'/>彩色</label></li>";
    ObjString += "<li>";
    ObjString += "<label for='Resolution'>分辨率:<select size='1' id='Resolution'><option value = ''></option></select></label></li>";
    ObjString += "</ul>";
...
function initDllForChangeImageSize() {
    ...
    vInterpolationMethod.options.add(new Option("近邻取样", 1));
    vInterpolationMethod.options.add(new Option("双线过滤", 2));
    vInterpolationMethod.options.add(new Option("双三次插值", 3));
...
function setDefaultValue() {
    ...
    var _strDefaultSaveImageName = "申请书";
        ...
        if (DWObject.ErrorCode == 0) {
            ...
            if(btnScan)
            {
                if ((vCount + vTWAINCount) == 0)
					...
					document.getElementById("btnScan").style.backgroundColor = "#00acd6";
